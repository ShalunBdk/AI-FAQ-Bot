...
### Каскадный поиск (4 уровня) + RAG генерация
- 🎯 **Exact Match** - точное совпадение (100%)
- 🔑 **Keyword Search** - поиск по ключевым словам с лемматизацией (70-95%)
  - Автоматическое распознавание всех форм слов (претензию → претензия)
  - Не требует ручного добавления всех склонений
- 🧠 **Semantic Search** - семантический поиск через AI (45-70%)
  - Модель: **deepvk/USER2-base** (русскоязычная, 8K контекст)
  - Улучшенное понимание перефразированных вопросов
- ❌ **Fallback** - вежливый отказ с подсказками
- 🤖 **RAG Generation** (опционально) - умная генерация ответов через LLM
  - Privacy First: анонимизация PII перед отправкой в облако
  - Объединение информации из нескольких FAQ
  - Адаптация ответа под конкретный вопрос
  - Поддержка различных LLM (GPT-4, Claude, Gemini и др.)

### Разрешение неоднозначностей (Disambiguation)
- 🔀 **Умное обнаружение** - активируется при разнице < 7% между топ-2 вариантами
- ✅ **Выбор пользователя** - бот предлагает 2-3 лучших варианта с кнопками
- 📊 **Полное логирование** - сохраняются реальные проценты уверенности (не 100%)
- 🎯 **Умная аналитика** - исключение из статистики "неудачных запросов"
- 📱 **Чистый UI** - редактирование сообщения вместо удаления (Bitrix24)

### Администрирование
- **Web-админка** - управление FAQ, аналитика, логи
  - Интерактивный просмотр вариантов disambiguation (клик для раскрытия деталей)
  - Отображение реальных процентов уверенности во всех логах
- **Оптимизация ключевых слов** - автоматическая очистка от дубликатов форм
- **Bitrix24 интеграция** - OAuth, встраивание в портал, система прав
- **Горячая перезагрузка** - обновление без перезапуска
- **Экспорт в CSV/PDF/Excel** - выгрузка статистики
...
- **AI/ML**:
  ...
  - sentence-transformers с моделью **deepvk/USER2-base** (149M параметров)
  - pymorphy3 (лемматизация для русского языка)
  - **RAG (Retrieval-Augmented Generation)**:
    - OpenRouter API (доступ к GPT-4, Claude, Gemini и др.)
    - natasha (NER для анонимизации PII)
    - Privacy First подход (анонимизация email, телефонов, имен)
- **Database**: SQLite
...
### Техническая (в docs/)
- **[RAG_GUIDE.md](docs/RAG_GUIDE.md)** - полное руководство по RAG (Privacy First)
- **[QUICKSTART_RAG.md](docs/QUICKSTART_RAG.md)** - быстрый старт с RAG за 5 минут
...
# RAG (опционально, для умной генерации ответов)
RAG_ENABLED=true                                    # включить/выключить RAG
OPENROUTER_API_KEY=sk-or-v1-xxxxx                 # ключ для OpenRouter
OPENROUTER_MODEL=openai/gpt-4o-mini               # модель LLM
RAG_MAX_TOKENS=1024                                # макс токенов в ответе
RAG_TEMPERATURE=0.3                                # температура генерации
RAG_MIN_RELEVANCE_SCORE=45.0                       # минимальный порог релевантности
RAG_MAX_CHUNKS=5                                   # макс документов для контекста
```
